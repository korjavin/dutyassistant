...
func TestTranslateToEnglish(t *testing.T) {
	...
	c := NewClient("test-key", server.URL, 10, "gpt-3.5-turbo", nil)
...
func TestTranslateToEnglish_ErrorCases(t *testing.T) {
	...
	c := NewClient("test-key", errorServer.URL, 10, "gpt-3.5-turbo", nil)
	res, err := c.TranslateToEnglish(ctx, text)
	if err == nil {
	...
	cTimeout := NewClient("test-key", timeoutServer.URL, 1, "gpt-3.5-turbo", nil)
