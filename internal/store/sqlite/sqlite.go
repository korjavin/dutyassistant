...
func (s *SQLiteStore) ListAllUsers(ctx context.Context) ([]*store.User, error) {
	query := `SELECT id, telegram_user_id, first_name, is_admin, is_active FROM users`
	rows, err := s.db.QueryContext(ctx, query)
	if err != nil {
...
func (s *SQLiteStore) CreateDuty(ctx context.Context, duty *store.Duty) error {
	...
	res, err := s.db.ExecContext(ctx, query, duty.UserID, duty.DutyDate, duty.AssignmentType, duty.CreatedAt.UTC().Format(time.RFC3339))
	if err != nil {
...
// GetDutyByDate retrieves a duty by its date.
func (s *SQLiteStore) GetDutyByDate(ctx context.Context, date string) (*store.Duty, error) {
	query := `SELECT id, user_id, duty_date, assignment_type, created_at FROM duties WHERE duty_date = ?`
	row := s.db.QueryRowContext(ctx, query, date)
	duty := &store.Duty{}
	var createdAtStr string

	err := row.Scan(&duty.ID, &duty.UserID, &duty.DutyDate, &duty.AssignmentType, &createdAtStr)
	if err != nil {
		if err == sql.ErrNoRows {
...
func (s *SQLiteStore) UpdateDuty(ctx context.Context, duty *store.Duty) error {
	...
	_, err := s.db.ExecContext(ctx, query, duty.UserID, duty.AssignmentType, duty.DutyDate)
	if err != nil {
...
func (s *SQLiteStore) DeleteDuty(ctx context.Context, date string) error {
...
func (s *SQLiteStore) GetDutiesByMonth(ctx context.Context, year int, month int) ([]*store.Duty, error) {
	startDate := fmt.Sprintf("%04d-%02d-01", year, month)
	endDate := time.Date(year, time.Month(month), 1, 0, 0, 0, 0, time.UTC).AddDate(0, 1, 0).Format("2006-01-02")

	query := `
		SELECT id, user_id, duty_date, assignment_type, created_at
		FROM duties
	...
	for rows.Next() {
		duty := &store.Duty{}
		var createdAtStr string
		err := rows.Scan(&duty.ID, &duty.UserID, &duty.DutyDate, &duty.AssignmentType, &createdAtStr)
		if err != nil {
...
func (s *SQLiteStore) IncrementAssignmentCount(ctx context.Context, userID int64) error {
	query := `
	...
	`
	_, err := s.db.ExecContext(ctx, query, userID, time.Now().UTC().Format(time.RFC3339))
	if err != nil {
