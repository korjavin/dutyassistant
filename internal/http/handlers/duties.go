...
func VolunteerForDuty(s store.Store) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		newDuty := &store.Duty{
			UserID:         user.ID,
			DutyDate:       dutyDate.Format("2006-01-02"),
		...
		_ = s.DeleteDuty(c.Request.Context(), newDuty.DutyDate)
...
func AdminModifyDuty(s store.Store) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		existingDuty, err := s.GetDutyByDate(c.Request.Context(), date)
...
func AdminDeleteDuty(s store.Store) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err := s.DeleteDuty(c.Request.Context(), date); err != nil {
