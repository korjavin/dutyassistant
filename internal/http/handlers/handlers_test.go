...
func TestGetSchedule(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		expectedDuties := []*store.Duty{
			{ID: 1, UserID: 101, DutyDate: "2023-10-25"},
		}

		mockStore.On("GetDutiesByMonth", mock.Anything, year, month).Return(expectedDuties, nil).Once()
	...
	t.Run("db error", func(t *testing.T) {
		mockStore.On("GetDutiesByMonth", mock.Anything, 2023, 11).Return(nil, errors.New("db error")).Once()
...
func TestVolunteerForDuty(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		mockStore.On("DeleteDuty", mock.Anything, date).Return(nil).Once()
		mockStore.On("CreateDuty", mock.Anything, mock.AnythingOfType("*store.Duty")).Return(nil).Once()

		body, _ := json.Marshal(gin.H{"date": date})
		w := httptest.NewRecorder()
...
func TestAdminAssignDuty(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		body, _ := json.Marshal(gin.H{"user_id": 101, "date": date})
		w := httptest.NewRecorder()
...
func TestAdminModifyDuty(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		adminUser := &store.User{ID: 1, TelegramUserID: 123, IsActive: true, IsAdmin: true}
		date := "2023-11-12"
		existingDuty := &store.Duty{ID: 1, UserID: 101, DutyDate: date}

		mockStore.On("GetDutyByDate", mock.Anything, date).Return(existingDuty, nil).Once()
		...
		w := httptest.NewRecorder()
		req, _ := http.NewRequest("PUT", "/api/v1/duties/"+date, bytes.NewBuffer(body))
		req.Header.Set("Content-Type", "application/json")
...
func TestAdminDeleteDuty(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		w := httptest.NewRecorder()
		req, _ := http.NewRequest("DELETE", "/api/v1/duties/"+date, nil)
