...
// HMACAuth returns a Gin middleware that verifies HMAC-SHA256 signed requests.
// It expects the caller to include two headers:
//   - X-Timestamp: Unix timestamp in milliseconds
//   - X-Signature: lowercase hex HMAC-SHA256(secret, timestamp)
//
// Requests with a timestamp older or newer than 5 minutes are rejected to
// prevent replay attacks.
//
// If secret is empty, the middleware always responds with 503 Service Unavailable,
// signalling that the endpoint is not configured.
